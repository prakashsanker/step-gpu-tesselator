...
// PATCHED: XcafFace now stores a parent shape pointer for color inheritance
...
public:
    ...
    // PATCHED: New constructor that accepts parent shape for color fallback
    XcafFace (const TopoDS_Face& face, const Handle (XCAFDoc_ShapeTool)& shapeTool, const Handle (XCAFDoc_ColorTool)& colorTool, const TopoDS_Shape* parent) :
        OcctFace (face),
        shapeTool (shapeTool),
        colorTool (colorTool),
        parentShape (parent)
    ...
    {
        // PATCHED: First try to get the face's own color
        if (GetShapeColor ((const TopoDS_Shape&) face, shapeTool, colorTool, color)) {
            return true;
        }
        // PATCHED: Fall back to parent shape's color if face doesn't have its own
        // This fixes STYLED_ITEM colors that are applied to solids/shells rather than individual faces
...
private:
    ...
    const TopoDS_Shape* parentShape;  // PATCHED: Added for color inheritance
};
...
public:
    ...
    virtual void EnumerateFaces (const std::function<void (const Face& face)>& onFace) const override
    {
        for (TopExp_Explorer ex (shape, TopAbs_FACE); ex.More (); ex.Next ()) {
            ...
            // PATCHED: Pass parent shape for color inheritance
            XcafFace outputFace (face, shapeTool, colorTool, &shape);
            onFace (outputFace);
        }
    }
