/**
 * Quick test to verify parallel ear clipping produces correct results
 */

import puppeteer from 'puppeteer';

async function main() {
    console.log('\n=== Testing Parallel Ear Clipping ===\n');
    ...
    page.on('console', msg => {
        const text = msg.text();
        if (text.includes('[EarClipping') || text.includes('Error')) {
    ...
    for (const test of tests) {
        try {
            const result = await page.evaluate(async (points) => {
                try {
                    const triangles = await window.benchmarkHarness.earClippingParallel(points);
