...
async function main() {
    ...
    await page.goto('http://localhost:5198/tests/test-harness.html', {
    ...
    // Test all B-spline files
    const bsplineDir = path.join(projectRoot, 'step-examples/c5-bspline');
    const files = fs.readdirSync(bsplineDir).filter(f => f.endsWith('.step'));

    console.log(`\n=== Testing ${files.length} B-spline files ===\n`);

    for (const file of files) {
        const stepFile = path.join(bsplineDir, file);
